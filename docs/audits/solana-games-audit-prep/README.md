...
3. **Close Call price source and Pyth correctness.** Current code passes open/close prices as instruction arguments supplied by the backend. Before external audit, Close Call should validate the Pyth BTC/USD price update on-chain against `CloseCallConfig.pyth_feed_id`, enforce freshness, and reject incompatible exponents. Until that lands, Close Call price sourcing is a backend trust assumption. After it lands, we still trust Pyth's published BTC/USD price as the canonical price during the audited freshness window and do not defend against a Pyth publisher that publishes a deliberately wrong price for a single update.
