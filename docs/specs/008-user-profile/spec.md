...
- **Endpoints (Iteration 1 — transaction history)**:
  - `GET  /profile/transactions?cursor=<id>&limit=<n>&game=<filter>` -- paginated transaction history for JWT-authenticated user (wallet resolved from profile). Default limit 20, max 50. Game filter accepts both DB names (`lord`, `closecall`) and frontend names (`lord-of-rngs`, `close-call`). Response: `{ transactions: [...], nextCursor }` with `amountLamports` as string
