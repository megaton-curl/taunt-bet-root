...
| L-020 | JSONB reward metadata | Insert JSONB via `JSON.stringify` params | Use the SQL client's structured JSON helper (`sql.json`) so JSONB operators/indexes see object keys. | database | active |
| L-021 | Float→`NUMERIC(p,s)` storage | Compute amount from full-precision float, then store both float and amount; storage rounds the float and the stored tuple no longer reproduces the math | When a derived value is stored at lower precision than the inputs, **quantize the input to the column's precision in code BEFORE using it for derived fields**. (`Math.round(x * 10**s) / 10**s` for NUMERIC(p,s).) Audit must reproduce: `amount = f(stored_inputs)` should be exact. | precision | active |
| L-022 | Tests asserted on literal rendered text (e.g. `"No campaigns rows."`) | Tests broke every time we changed empty-state copy or formatted output during the peek 403 redesign. ~6 separate test-alignment commits across phases 1-5. | Assert against role/structure (`getByRole`, `getByLabelText`, columnheader names) and against semantic markers (presence of a `<code>` element naming the source table), not against literal user-facing strings. Reserve text assertions for the rare case where the copy is the contract. | testing | active |
| L-023 | Code referenced a dropped table (`dogpile_events`) | Phase 3 added an `isRetiredDogpileError` defensive guard that silenced the missing-table error rather than deleting the dead query. Phase 4 then deleted both the dead code and the guard. | When code references a dropped/retired resource, audit and delete the references — don't paper over with a try/catch guard. Defensive coding around dead state is debt with no upside. | dead-code | active |
| L-024 | Stalled subagent partial work + local `pnpm verify` exit-0 | CI failed twice on the same peek redesign after I committed a stalled subagent's partial state. Local `pnpm verify` reported pass; CI caught test misses in the same vitest invocation. Vitest parallelism or resource flake suspected. | After a subagent takeover (committing partial work), re-run `pnpm test <path>` *per touched file* before staging the commit. Don't rely solely on `pnpm verify` exit-0 to gate work that was never run-to-completion by its author. | subagent-takeover | active |
